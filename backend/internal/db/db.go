...
func RunMigrations() error {
	...
	// Curated EMBEDDED FALLBACK advertiser + campaigns so the ad feed works
	// before any real advertiser registers. Fixed UUIDs keep re-runs idempotent.
	const fallbackAdvertiserID = "11111111-1111-1111-1111-111111111111"
	if _, err := Pool.Exec(ctx, `
		INSERT INTO advertisers (id, name, verified)
		VALUES ($1, 'Moonbug Curated', TRUE)
		ON CONFLICT (id) DO UPDATE SET verified = TRUE
	`, fallbackAdvertiserID); err != nil {
		return fmt.Errorf("seeding fallback advertiser failed: %w", err)
	}
	if _, err := Pool.Exec(ctx, `
		INSERT INTO ad_campaigns
			(id, advertiser_id, format, title, payload_url, reward_per_action, reward_currency, target_categories, nsfw, status)
		VALUES
			('22222222-2222-2222-2222-222222222222', $1, 'picture', 'Cosmic Calm', 'https://cdn.moonbug.app/fallback/cosmic-calm.jpg', 0.05, 'USDC', '["influential","relaxed"]'::jsonb, FALSE, 'active'),
			('33333333-3333-3333-3333-333333333333', $1, 'video', 'Lunar Glow', 'https://cdn.moonbug.app/fallback/lunar-glow.mp4', 0.10, 'USDC', '["humorous","creative"]'::jsonb, FALSE, 'active')
	...
	`, fallbackAdvertiserID); err != nil {
		return fmt.Errorf("seeding fallback campaigns failed: %w", err)
	}
